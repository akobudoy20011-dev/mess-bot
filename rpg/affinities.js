...
function normalizeAffinityId(value) {
  ...
  if (Object.prototype.hasOwnProperty.call(AFFINITIES, raw)) {
    return raw;
  }
...
function getAffinityKingdom(affinityId) {
  const affinity = getAffinity(affinityId);
...
function getTier(tier) {
  if (tier && typeof tier === "object") {
    if (Number.isFinite(Number(tier.id))) {
      tier = Number(tier.id);
    } else if (tier.key) {
      tier = tier.key;
    }
  ...
  if (typeof tier === "string") {
    ...
    if (!/^\d+$/.test(trimmed)) {
      return getTierByKey(trimmed);
    }

    tier = Number(trimmed);
  }

  const numeric = Number.isFinite(Number(tier))
    ? Math.floor(Number(tier))
    : 0;

  const clamped = Math.max(0, Math.min(6, numeric));

  return TIER_ORDER[clamped] || AFFINITY_TIERS.NONE;
}
...
function getNextTier(tier) {
  ...
  return TIER_ORDER[current.id + 1] || null;
}
...
async function getPlayerAffinity(
  ...
  return normalizePlayerAffinityRow(rows[0]);
}

function normalizePlayerAffinityRow(row) {
  if (!row) {
    return null;
  }
  ...
  const tier = Number.isFinite(rawTier)
    ? Math.max(0, Math.min(6, Math.floor(rawTier)))
    : 0;

  const rawMastery = Number(row.mastery);

  const mastery =
    Number.isFinite(rawMastery) && rawMastery >= 0
      ? Math.floor(rawMastery)
      : 0;

  return {
    ...row,

    affinity_id: normalizeAffinityId(
      row.affinity_id
    ),

    tier,
    tierId: tier,
    tierKey: getTier(tier).key,
    ...
    primaryAffinity:
      ...
      row.primary_affinity === "true" ||
      row.primary_affinity === 1,

    powerMultiplier:
      getTier(tier).powerMultiplier,
  };
}
...
async function getPrimaryAffinity(
  ...
  return rows.length
    ? normalizePlayerAffinityRow(rows[0])
    : null;
}
...
async function setPrimaryAffinity(
  ...
  const state = await getPlayerAffinity(
    threadId,
    userId,
    id
  );
  ...
  /*
   * Clear every current primary first.
   */
  await db.query(
    `
  ...
  /*
   * Set the requested affinity as primary.
   */
  await db.query(
    `
      UPDATE rpg_player_affinities
...
async function unlockAffinity(
  ...
  const definition = AFFINITIES[id];

  const existing = await getPlayerAffinity(
    threadId,
    userId,
    id
  );
  ...
  const requestedTier = Math.max(
    ...
    Math.min(
      6,
      Number.isFinite(Number(options.tier))
        ? Math.floor(Number(options.tier))
        : 1
    )
  );

  const suppliedMastery =
    Number(options.mastery);

  const mastery =
    Number.isFinite(suppliedMastery)
      ? Math.max(
          0,
          Math.floor(suppliedMastery)
        )
      : 0;
  ...
  if (!createdState) {
    throw new Error(
      `Failed to unlock ${definition.name} affinity.`
    );
  }
...
function getEnvironmentMultiplier(
  ...
  const specialTimeWeather = new Set([
    "night",
    "morning",
  ]);
  ...
  if (
    affinity.weather &&
    currentWeather &&
    !specialTimeWeather.has(currentWeather) &&
    Object.prototype.hasOwnProperty.call(
      affinity.weather,
      currentWeather
    )
...
function formatAffinity(
  ...
  const primary =
    state.primaryAffinity === true ||
    state.primary_affinity === true
      ? " • PRIMARY"
      : "";
