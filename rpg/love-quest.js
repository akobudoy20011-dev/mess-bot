...
async function getQuest(threadID, playerID) {
  ...
  const result = await db.query(
    `SELECT thread_id, player_id, quest_id, chapter, stage, status, choice, started_at, updated_at, completed_at 
     FROM rpg_special_quests 
     WHERE thread_id = $1 AND player_id = $2 AND quest_id = $3 
     LIMIT 1`,
    [
      String(threadID),
      String(playerID),
...
async function makeChoice(
  ...
  const choiceMessages = {
    follow: "You decided to follow the distant light.",
    hesitate: "You hesitated, but the star remained.",
    reach: "You reached toward the distant star.",
    wait: "You chose to wait beneath the night sky.",
    fight: "You chose to fight for what mattered.",
    stay: "You chose to stay.",
    flow: "You chose to keep moving with the river.",
    stop: "You chose to stop and look back.",
    home: "You chose the home you imagined together.",
    stars: "You chose to keep looking toward the stars.",
    forever: "You chose forever.",
    moment: "You chose to treasure the moment.",
  };
  ...
  await send(
    ...
    [
      "✦ THE LAST STAR ✦",
      "",
      choiceMessages[cleanChoice] || `You chose: ${cleanChoice}`,
      "",
