...
async function handleGameCommand(api, event, command, args) {
  ...
  if (cmd === "answer") {
    const handled = await examManager.handleExamResponse(
      api,
      event,
      normalizedArgs.join(" "),
      normalizedArgs.join(" ")
    ...
    await safeReply(api, event, "♡ ECLIPSE\n\nnothing is waiting for an answer right now.\n\ntry !exam to begin an examination ♡");
    return true;
  }
  ...
  if (cmd === "exam") {
    return examManager.handleExam(api, event, normalizedArgs);
  }
